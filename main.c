...
//  volcafmpatchdecoder
//
//  Updated by Dwayne Myers on 10/27/22.
//
...
{
  //Geneate a single patch sysex file
  int i;
  char sum;
...
int main(int argc, char *argv[])
{
  ...
  for( fa = 1; fa < argc; fa++ )
  {
    ...
    //*************
    //Read file length to determine if it is greater than a 32 patch FM1 file
    ...
    //**Print file length for debugging FM2
    printf("File Length: %d", size);
    printf("\n");
    
    //Read WAV file header and check that it is the proper format
    ...
    if( ( res = malloc( frames / 8 ) ) == NULL || ( samples = malloc( frames * wh->channels * sizeof( short ) ) ) == NULL )
      {
        ...
        {
            if (res[5] == 0x00) // CARTRIDGE
            {
                int i, j;
                signed char sum;
                
                decode(res,  2, infile);
                p = strcat(fname, "_cart.syx");
                outfile = fopen(p, "wb" );
            ...
            if (size > 5750000) //FM2 - Second Cartridge
            {
                int i, j;
                signed char sum;
                printf("Second Cartridge Found - FM2\n");
                
                decode(res,  2, infile);
                p = strcat(fname2, "_cart2.syx");
                //strcpy( p, "_cart.syx" );
                outfile = fopen(p, "wb" );
                fwrite(&SYSEX_HEADER_4096, sizeof(SYSEX_HEADER_4096), 1, outfile);
                sum=0;
